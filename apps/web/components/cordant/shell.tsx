...
const WORKSPACE_NAV = [
	...
	},
	{
		href: "/demo/tickets",
		label: "Tickets",
		icon: <path d="M4 6.5h16v11H4v-11zM4 10.5h16M8 14.5h4" />,
	},
	{
		href: "/demo/projects",
		label: "Projects",
		icon: <path d="M4 5.5h6l2 2.5h8v10.5H4v-13z" />,
	},
	{
		href: "/demo/automation",
		label: "Automation",
		icon: (
			<path d="M12 3v3.5M12 17.5V21M4.5 12H8M16 12h3.5M6.3 6.3l2.5 2.5M15.2 15.2l2.5 2.5M17.7 6.3l-2.5 2.5M8.8 15.2l-2.5 2.5" />
		),
	},
];
...
export function CordantShell({ children }: { children: ReactNode }) {
	...
	return (
		<div className="flex min-h-screen w-full overflow-x-hidden bg-zinc-50 text-zinc-900">
			<aside className="fixed inset-y-0 left-0 z-30 flex w-60 flex-col border-r border-zinc-200 bg-white max-md:hidden">
				<div
					className="flex items-center gap-2.5 border-b border-zinc-100 px-5"
					style={{ height: 60 }}
				>
				...
				<nav className="flex-1 overflow-y-auto px-3 py-5">
					{context === "admin" && (
						<>
							<Link
								href="/demo"
								className="mb-4 flex items-center gap-1.5 px-2.5 text-[12px] font-medium text-zinc-500 transition hover:text-zinc-800"
							>
								<svg
									viewBox="0 0 24 24"
									width="13"
					...
					{context === "workspace" && (
						<>
							...
							<p className="mb-1.5 mt-6 px-2.5 text-[10.5px] font-bold uppercase tracking-wider text-zinc-400">
								Shortcuts
							</p>
							<Link
								href="/demo/settings/profile"
								className="mb-0.5 flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13.5px] font-medium text-zinc-600 transition hover:bg-zinc-50 hover:text-zinc-900"
							>
				...
				<div className="border-t border-zinc-100 p-4">
					<div className="flex items-center gap-2.5">
						<span className="grid h-8 w-8 flex-none place-items-center rounded-full bg-gradient-to-br from-violet-500 to-blue-500 text-[12px] font-bold text-white">
							D
						</span>
						<div className="min-w-0">
			...
			<div className="min-w-0 flex-1 md:pl-60">
				<header
					className="sticky top-0 z-20 flex items-center gap-2 border-b border-zinc-200 bg-white/85 px-4 backdrop-blur-lg md:gap-4 md:px-8"
					style={{ height: 60 }}
				>
					<div className="flex w-0 min-w-0 flex-1 items-center gap-2 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-zinc-400 md:max-w-md">
						<svg
							viewBox="0 0 24 24"
							width="14"
							height="14"
						...
						<span className="truncate text-[12.5px]">
							Search tickets, projects, people…
						</span>
					</div>
					<p className="hidden min-w-0 truncate text-[12.5px] text-zinc-400 xl:block">
						Live demo — try asking Otter{" "}
						<em className="text-zinc-600">"enable 2FA"</em> and see what it
						finds.
					</p>
					<Link
						href="/"
						className="shrink-0 text-[13px] font-semibold text-zinc-400 transition hover:text-zinc-900"
					>
						← otter.dev
					</Link>
				</header>
				<nav className="sticky top-[60px] z-10 flex flex-wrap gap-2 border-b border-zinc-200 bg-white px-4 py-2 md:hidden">
					{context !== "workspace" && (
						<Link
							href="/demo"
							className="shrink-0 rounded-lg px-3 py-1.5 text-[12.5px] font-semibold text-zinc-500"
						>
							Workspace
						</Link>
					)}
				...
				<main className="mx-auto w-full max-w-4xl px-4 pb-28 pt-7 sm:px-6 md:px-10 md:py-9">
					{children}
				</main>
			</div>
		</div>
	...
	return (
		...
		>
			{icon && (
				<svg
					viewBox="0 0 24 24"
					className={`h-[17px] w-[17px] flex-none ${active ? "text-indigo-600" : "text-zinc-400"}`}
					aria-hidden="true"
					focusable="false"
					fill="none"
					stroke="currentColor"
