...
export function Markdown({ source }: { source: string }) {
	...
	while (index < lines.length) {
		...
		if (
			...
			blocks.push(
				<div className="mk-table-wrap" key={`table-${index}`}>
					<table>
						<thead><tr>{headers.map((cell, cellIndex) => <th key={`${cellIndex}-${cell}`}>{inline(cell)}</th>)}</tr></thead>
						<tbody>{body.map((row, rowIndex) => <tr key={`row-${rowIndex}`}>{row.map((cell, cellIndex) => <td key={`${cellIndex}-${cell}`}>{inline(cell)}</td>)}</tr>)}</tbody>
					</table>
				</div>,
			);
			continue;
