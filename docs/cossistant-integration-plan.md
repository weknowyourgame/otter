# Otto ← Cossistant integration plan

What we're copying from `cossistant/` into Otto, and what we're skipping. Cossistant's code is AGPL-3.0 (free for non-commercial use, paid license for commercial) — check which one Otto is before copying actual source, not just the idea.
...
|---|---|
| `apps/api/src/ws/socket.ts` | Turns Otto's polling into a live socket connection. |
...
| `apps/api/src/ws/realtime-pubsub.ts` | Lets live updates work even if Otto runs on more than one server. |
...
| `apps/api/src/ai-pipeline/shared/safety/kill-switch.ts` | An instant off-switch for the whole agent, no redeploy needed — important since Otto clicks around real UIs. |

### Logins & access
| Source | Why we're copying it |
|---|---|
| `apps/api/src/lib/auth.tsx` | Real login system, replaces Otto's fake dashboard key. |
...
| Source | Why we're copying it |
|---|---|
| `packages/browser/src/embed/loader.ts` | The small script that loads when someone drops Otto's `<script>` tag on their site. |
...
| `scripts/prepare-package.ts` | Packages `otto-core`/`otto-sdk` for publishing to npm. |

### Answering questions (not just doing tasks)
| Source | Why we're copying it |
|---|---|
| `apps/api/src/db/schema` (docs table) | Place to store a customer's help docs so Otto can answer from them. |
...
| `apps/api/src/ai-pipeline/shared/knowledge-gap/` | Makes Otto say "I don't know" instead of making something up. |
...
| `apps/api/src/support-capabilities/knowledge.ts` | Limits what Otto is allowed to answer about, per customer. |
...
| `apps/workers` folder layout | Copy the pattern (one worker file per job type) for whatever connectors Otto adds later. |

### Memory across sessions
| Source | Why we're copying it |
|---|---|
| `packages/memory` | Lets Otto remember facts about a user between visits (e.g. "already turned on 2FA last week") using the same tool-calling style Otto's engine already uses. |

### Shared setup / dev tooling
| Source | Why we're copying it |
|---|---|
| `packages/types/src/realtime-events.ts` (pattern) | Template for defining Otto's own list of event types shared between server and widget. |
...
| `packages/location` | Only needed if we translate Otto's answers into other languages later. |
| `infra/aws/s3-public-setup` | Only needed if Otto should support file uploads (e.g. screenshots). |

## Not cloning

| Source | Why we're skipping it |
|---|---|
| `apps/api/src/mcp/` | Cool feature (lets other AI tools call Otto) but not needed to get act + answer working — later. |
| `apps/api/src/polar/`, `lib/plans`, `lib/team-seats.ts` | This is Cossistant's billing system. Otto doesn't have pricing plans yet. |
| `tinybird/` | Cossistant's analytics dashboard backend — built for their metrics, not something Otto needs. |
| `apps/api/src/lifecycle-email/` | Automated marketing emails ("come back and try us"). Not relevant to an agent that does tasks. |
| `apps/api/src/mail/`, `resend/`, `ses/`, `infra/aws/ses-email-setup` | Otto doesn't send/receive email yet, so there's nothing for this to plug into. |
| `apps/facehash-landing`, `packages/facehash` | A completely different product (an avatar generator) bundled in the same repo. Not related to Otto at all. |
| `packages/tiny-markdown` | A markdown renderer for Cossistant's chat. Otto doesn't need this. |
| `packages/transactional` | Was empty when we checked, and tied to the email system we're skipping anyway. |
| `examples/nextjs-tailwind`, `examples/react-vite` | Sample apps showing how to install `@cossistant/react`. Otto has its own SDK, doesn't apply. |
| `apps/api/src/ai-pipeline` (everything besides generation/knowledge-gap) | The rest is Cossistant's respond-or-stay-silent decision logic (should the bot even reply to this message) — not relevant to Otto, which always acts on an explicit user request. |

## Ground rules for the build (read before every phase)

- **This repo has other agents working in it.** A separate agent owns `apps/web/app/(marketing)/**` (landing page: `page.tsx`, its own `layout.tsx`, `globals.css`, `otto-runtime.tsx`, `public/mirage*`). Never edit, `git add`, or commit those paths. See `[[otto-monorepo-parallel-agents]]` / `[[otto-shared-globals-css-risk]]` in project memory.
